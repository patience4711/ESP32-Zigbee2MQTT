...
server.on("/DEVSCRIPT", HTTP_GET, [](AsyncWebServerRequest *request) {
   request->send_P(200, "text/css", DEV_SCRIPT);
});

server.on("/DEV_CONFIG", HTTP_GET, [](AsyncWebServerRequest *request) {
  if(checkRemote( request->client()->remoteIP().toString()) ) request->redirect( "/DENIED" );  
  devChoice=0;
  
  strcpy( requestUrl, request->url().c_str() ); 
  deviceForm(); // prepare the page part with the form
  request->send_P(200, "text/html", DEVCONFIG_START, processor);
});
...
//     consoleOut("devChoice at enter = " + String(devChoice));
//     if( devChoice == 99 ) {
//         devChoice = deviceCount; //indicate this is an adition
//         deviceCount += 88;
//         }
//      String bestand = "/Dev_Prop" + String(devChoice) + ".str";
//      consoleOut("devChoice = " + String(devChoice));
//      if (!SPIFFS.exists(bestand)) Dev_Prop[devChoice].devType = 2;
//      deviceForm(); // prepare the form page
//      request->send_P(200, "text/html", DEVCONFIG_START, processor); //send the html code to the client
// });
...
server.on("/CONTROL", HTTP_GET, [](AsyncWebServerRequest *request) {
    devChoice = atoi(request->arg("dev").c_str()) ;
    request->send_P(200, "text/html", BULB_CONTROL);
 });
server.on("/ONOFF", HTTP_GET, [](AsyncWebServerRequest *request) {
    devChoice = atoi(request->arg("dev").c_str()) ;
    request->send_P(200, "text/html", SWITCH_PAGE);
 });
...
handleDim(request) ; // request looks like /DIM?
request->send_P(200, "text/html", "");
});

server.on("/BUTTON", HTTP_GET, [](AsyncWebServerRequest *request) {
handleButtons(request) ;
request->send_P(200, "text/html", "");
});
...
server.on("/DEV", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    consoleOut("DEV devChoice at enter = " + String(devChoice));
    if( devChoice == 99 ) {
        devChoice = deviceCount; //indicate this is an adition
        deviceCount += 88;
        }
     String bestand = "/Dev_Prop" + String(devChoice) + ".str";
     consoleOut("devChoice = " + String(devChoice));
     if (!SPIFFS.exists(bestand)) Dev_Prop[devChoice].devType = 0;
     deviceForm(); // prepare the form page
     request->send_P(200, "text/html", DEVCONFIG_START, processor); //send the html code to the client
});

// server.on("/CONFIRM_DEV", HTTP_GET, [](AsyncWebServerRequest *request) {
//    toSend = FPSTR(CONFIRM_DEV); // prepare the page
...
server.on("/get.Joined", HTTP_GET, [](AsyncWebServerRequest *request) {     
  consoleOut("get.Joined devChoice at enter = " + String(dev2Join));
// set the array into a json object
  String json="{";
  json += "\"dev2Join\":\"" + String(dev2Join) + "\",";
  json += "\"devAdr\":\"" + String(Dev_Prop[dev2Join].devAdr) + "\"";
  json += "}";
